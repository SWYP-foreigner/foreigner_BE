...
public class OnboardingVisitorShareWeeklyScheduler {
    ...
    public void refresh() {
        if (weeklyGauge == null) {
            weeklyGauge = MultiGauge.builder("onboarding_visitor_share_weekly")
                    .description("Weekly VISITOR / Total (calendar week, KST)")
                    .register(registry);
        }
        ...
        for (Object[] r : rows) {
            ...
            double visitors = asDouble(r[2]);
            double share    = total > 0 ? (visitors / total) : 0.0;
